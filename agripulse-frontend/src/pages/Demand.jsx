...
import { ClipboardDocumentListIcon, PlusIcon, ArrowPathIcon, FunnelIcon, XMarkIcon, ArrowUpIcon } from "@heroicons/react/24/outline";
...
export default function DemandPage() {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 md:gap-0 md:-mx-4">
      {/* Create Form Button - Mobile: Top/Sticky, Desktop: Right Sidebar/Fixed */}
      <div className="order-1 md:order-3 md:w-0 md:flex-shrink-0 md:overflow-hidden">
        ...
        <div className="md:hidden fixed left-0 right-0 z-40 px-4" data-create-form-mobile style={{ top: '72px' }}>
          {!showCreateForm ? (
            <button
              ...
              className="w-full bg-gradient-to-r from-emerald-600 to-green-600 text-white py-3 rounded-xl font-bold shadow-lg hover:shadow-xl hover:from-emerald-700 hover:to-green-700 transition-all flex items-center justify-center gap-2"
            >
              <PlusIcon className="w-5 h-5" />
              <span className="text-sm">Make your orders</span>
            </button>
          ) : (
            <div className="bg-white rounded-xl shadow-lg border border-slate-100 flex flex-col overflow-hidden" style={{ maxHeight: 'calc(100vh - 10rem)' }}>
              <div className="p-4 pb-3 flex-shrink-0 border-b border-slate-200 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <h3 className="text-lg font-bold text-slate-800">Make your orders</h3>
                </div>
                <button
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border-2 border-slate-200 rounded-lg px-3 py-2 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all"
                  placeholder="0"
                  min="0"
                  required
                />
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Price Offer (KES)</label>
                <input
                  type="number"
              ...
              />
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Pickup Radius (Km)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
                  Reference Images (Optional) <span className="text-slate-500 font-normal">- similar/like this one</span>
                </label>
                <input
                  type="file"
                  accept="image/*"
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full bg-gradient-to-r from-emerald-600 to-green-600 text-white py-2.5 rounded-lg font-semibold text-sm shadow-md hover:shadow-lg hover:from-emerald-700 hover:to-green-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {submitting ? (
                  <>
        ...
        >
            <div className="p-6 pb-4 flex-shrink-0 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <h3 className="text-xl font-bold text-slate-800">Make your orders</h3>
              </div>
            </div>
            ...
            />
            <div>
              <label className="block text-xs md:text-sm font-semibold text-slate-700 mb-1 md:mb-1.5">Quantity Needed (Kg) *</label>
              <input
                type="number"
                value={form.qtyKg || ""}
                onChange={(e)=>setForm({...form, qtyKg: e.target.value === "" ? "" : Number(e.target.value)})}
                className="w-full border-2 border-slate-200 rounded-lg px-3 md:px-4 py-2 md:py-2.5 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all"
                placeholder="0"
                min="0"
                required
              />
            ...
            <div>
              <label className="block text-xs md:text-sm font-semibold text-slate-700 mb-1 md:mb-1.5">Price Offer (KES per kg)</label>
              <input
                type="number"
                value={form.priceOffer || ""}
                onChange={(e)=>setForm({...form, priceOffer: e.target.value === "" ? "" : Number(e.target.value)})}
            ...
            <div>
              <label className="block text-xs md:text-sm font-semibold text-slate-700 mb-1 md:mb-1.5">Urgency</label>
              <select
                value={form.urgency}
                onChange={(e)=>setForm({...form, urgency:e.target.value})}
                className="w-full border-2 border-slate-200 rounded-lg px-3 md:px-4 py-2 md:py-2.5 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all"
            ...
            />
            <div>
              <label className="block text-xs md:text-sm font-semibold text-slate-700 mb-1 md:mb-1.5">Pickup Radius (Km)</label>
              <input
                type="number"
                value={form.preferredPickupRadiusKm || ""}
                onChange={(e)=>setForm({...form, preferredPickupRadiusKm: e.target.value === "" ? "" : Number(e.target.value)})}
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full bg-gradient-to-r from-emerald-600 to-green-600 text-white py-2.5 md:py-3 rounded-lg font-semibold text-sm md:text-base shadow-md hover:shadow-lg hover:from-emerald-700 hover:to-green-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {submitting ? (
                <>
      ...
      {/* Filters Sidebar - Collapsible on mobile, always expanded and fixed on PC */}
      ...
      {/* Main Content */}
      <div className="order-3 md:order-2 flex-1 md:ml-[17rem] md:mr-[21rem] pt-40 md:pt-24">
        <div className="flex justify-between items-center mb-4 md:mb-6">
          <div className="flex items-center gap-2 md:gap-3">
            <div className="p-1.5 md:p-2 bg-emerald-100 rounded-lg">
              <ClipboardDocumentListIcon className="w-4 h-4 md:w-6 md:h-6 text-emerald-600" />
            </div>
            <h2 className="text-xl md:text-3xl font-bold text-slate-800">Market Demands</h2>
            {pagination.total > 0 && (
          ...
          <button
            onClick={load}
            disabled={loading}
            className="p-1.5 md:p-2 text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all"
            title="Refresh"
          >
          ...
          {loading ? (
            ...
            demands.map(d => (
              <DemandCard 
                key={d._id} 
                item={d} 
                onOfferSupply={handleOfferSupply}
                isExpanded={expandedCardId === d._id}
      ...
      {showOfferModal && selectedDemand && (
        ...
        >
          ...
          >
            <div className="p-6 border-b border-slate-200 flex justify-between items-center sticky top-0 bg-gradient-to-r from-emerald-50 to-white z-10">
      ...
      {showScrollTop && (
        <button
          ...
          className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 bg-gradient-to-r from-emerald-600 to-green-600 text-white p-3 rounded-full shadow-lg hover:shadow-xl hover:from-emerald-700 hover:to-green-700 transition-all flex items-center justify-center"
