...
import { MapPinIcon, CurrencyDollarIcon, ScaleIcon, ShoppingCartIcon, ChevronUpIcon, UserIcon, PhotoIcon, EyeIcon, ClockIcon, ChatBubbleLeftRightIcon, PaperAirplaneIcon } from "@heroicons/react/24/outline";
import { CheckBadgeIcon as CheckBadgeIconSolid } from "@heroicons/react/24/solid";
import { incrementListingViews } from "../api/produce";
...
export default function ProduceCard({ item, onBuyNow, showActions = true, isExpanded: isExpandedProp, onExpand }) {
  ...
  const handleCardClick = async (e) => {
    ...
    if (!wasExpanded && item._id) {
      await incrementListingViews(item._id);
    }
  };
  ...
  const loadOrCreateMatch = async () => {
    try {
      ...
      // Check if user is the farmer (can't message themselves)
      if (currentUser._id === (item.farmerId?._id || item.farmerId)) {
        return;
      }
      ...
      const existingMatch = matchesList.find(m => 
        m.listing?._id === item._id || m.listingId === item._id
      );

      if (existingMatch) {
        setMatchId(existingMatch._id);
      } else {
        // No existing match - user needs to create one via Buy Now or Matches page
        setMatchId(null);
      }
    } catch (err) {
  ...
  return (
    <div 
      ...
      className={`bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 border border-slate-200 hover:border-green-300 group cursor-pointer overflow-hidden ${isExpanded ? 'md:col-span-full' : ''}`}
    >
      {/* Hero Image Section */}
      {primaryImage ? (
        <div className="relative h-32 md:h-36 w-full overflow-hidden bg-gradient-to-br from-green-100 to-emerald-100">
          <img 
            src={primaryImage} 
            alt={item.crop}
          ...
          <div className="absolute top-2 right-2 md:top-3 md:right-3 bg-white/90 backdrop-blur-sm px-2 py-1 md:px-3 md:py-1.5 rounded-full">
            <div className={`text-[10px] md:text-xs font-bold ${item.status === 'available' ? 'text-green-600' : 'text-slate-500'}`}>
              {item.status?.toUpperCase() || 'AVAILABLE'}
            </div>
          </div>
          {item.category && (
            <div className="absolute top-2 left-2 md:top-3 md:left-3 bg-green-600/90 backdrop-blur-sm text-white px-2 py-1 md:px-3 md:py-1.5 rounded-full text-[10px] md:text-xs font-semibold">
              {item.category}
            </div>
          )}
        </div>
      ) : (
        <div className="relative h-32 md:h-36 w-full bg-gradient-to-br from-green-100 via-emerald-50 to-green-100 flex items-center justify-center">
          <PhotoIcon className="w-12 h-12 md:w-16 md:h-16 text-green-300" />
          <div className="absolute top-2 right-2 md:top-3 md:right-3 bg-white/90 backdrop-blur-sm px-2 py-1 md:px-3 md:py-1.5 rounded-full">
            <div className={`text-[10px] md:text-xs font-bold ${item.status === 'available' ? 'text-green-600' : 'text-slate-500'}`}>
              {item.status?.toUpperCase() || 'AVAILABLE'}
            </div>
      ...
      <div className="p-3 md:p-4">
        <div className="mb-2 md:mb-3">
          <h3 className="text-base md:text-lg font-bold text-slate-800 group-hover:text-green-700 transition-colors mb-1 md:mb-1.5 line-clamp-1">
            {item.crop}
          </h3>
          <div className="flex items-center gap-2 md:gap-2.5 text-xs md:text-sm text-slate-600">
            <div className="flex items-center gap-1">
              <ScaleIcon className="w-3.5 h-3.5 md:w-4 md:h-4 text-green-600" />
              <span className="font-semibold text-slate-800">{item.quantityKg} kg</span>
            </div>
            <span className="text-slate-400">•</span>
            <div className="flex items-center gap-1">
              <CurrencyDollarIcon className="w-3.5 h-3.5 md:w-4 md:h-4 text-green-600" />
              <span className="font-semibold text-slate-800 text-xs">
                {item.expectedPrice ? (
                  <>
                    <span>KES {item.expectedPrice.toLocaleString()}/kg</span>
                    <span className="text-slate-500 ml-1">({(item.expectedPrice * item.quantityKg).toLocaleString()} total)</span>
                  </>
                ) : "Negotiable"}
              </span>
            </div>
        ...
        <div className="flex items-center gap-2 md:gap-2.5 mb-2 md:mb-3 pb-2 md:pb-3 border-b border-slate-100">
          {profilePic ? (
            <img 
              src={profilePic} 
              alt={farmerName}
              className="w-7 h-7 md:w-8 md:h-8 rounded-full object-cover border-2 border-green-200 ring-1 ring-green-50"
            />
          ) : (
            <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-gradient-to-br from-green-400 to-emerald-500 flex items-center justify-center ring-1 ring-green-50">
              <UserIcon className="w-4 h-4 md:w-5 md:h-5 text-white" />
            </div>
          )}
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="font-semibold text-xs md:text-sm text-slate-800 truncate">{farmerName}</span>
              {isFarmerVerified ? (
                <span className="px-1.5 py-0.5 bg-green-100 text-green-700 rounded-full text-[10px] md:text-xs font-bold flex items-center gap-0.5 flex-shrink-0">
            ...
            <div className="flex items-center gap-1.5 text-[10px] md:text-xs text-slate-600">
              ...
              {farmer?.rating && <span className="text-slate-400">•</span>}
              <div className="flex items-center gap-0.5 truncate">
                <MapPinIcon className="w-3 h-3 md:w-3.5 md:h-3.5 text-green-600 flex-shrink-0" />
      ...
      {isExpanded && (
        <div className="border-t-2 border-slate-200 bg-gradient-to-b from-slate-50 to-white p-3 md:p-4 space-y-3 md:space-y-4 animate-in slide-in-from-top-2">
          ...
          {item.images && item.images.length > 0 && (
            <div className="bg-white p-3 md:p-4 rounded-lg md:rounded-xl border border-slate-200 shadow-sm">
              <h4 className="font-bold text-sm md:text-base text-slate-800 mb-2 md:mb-3 flex items-center gap-1.5 md:gap-2">
                <PhotoIcon className="w-4 h-4 md:w-5 md:h-5 text-green-600" />
          ...
          {/* Product Details & Seller Info - Side by Side */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
            {/* Product Specifications */}
            <div className="bg-white p-3 md:p-4 rounded-lg md:rounded-xl border border-slate-200 shadow-sm">
              <h4 className="font-bold text-sm md:text-base text-slate-800 mb-2 md:mb-3">Product Details</h4>
              <div className="grid grid-cols-2 gap-2 md:gap-2.5">
                <div className="bg-slate-50 p-2 md:p-2.5 rounded-lg">
                  <div className="text-[10px] md:text-xs text-slate-500 mb-0.5 md:mb-1">Variety</div>
                ...
                <div className="bg-slate-50 p-2 md:p-2.5 rounded-lg">
                  ...
                  <div className="font-bold text-xs md:text-sm text-slate-800">{item.quantityKg} kg</div>
                </div>
                <div className="bg-slate-50 p-2 md:p-2.5 rounded-lg">
                  <div className="text-[10px] md:text-xs text-slate-500 mb-0.5 md:mb-1">Price</div>
                  <div className="font-bold text-xs md:text-sm text-slate-800">
                    {item.expectedPrice ? (
                      <>
                        <div>KES {item.expectedPrice.toLocaleString()}/kg</div>
                        <div className="text-[10px] text-slate-600 mt-0.5">Total: KES {(item.expectedPrice * item.quantityKg).toLocaleString()}</div>
                      </>
                    ) : "Negotiable"}
                  </div>
                </div>
                {item.readyDate && (
                  <div className="bg-slate-50 p-2 md:p-2.5 rounded-lg col-span-2">
                    <div className="text-[10px] md:text-xs text-slate-500 mb-0.5 md:mb-1">Ready Date</div>
                    <div className="font-bold text-xs md:text-sm text-slate-800">
                      {new Date(item.readyDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </div>
                  </div>
                )}
                <div className="bg-slate-50 p-2 md:p-2.5 rounded-lg col-span-2">
            ...
            <div className="bg-white p-3 md:p-4 rounded-lg md:rounded-xl border border-slate-200 shadow-sm">
              <h4 className="font-bold text-sm md:text-base text-slate-800 mb-2 md:mb-3 flex items-center gap-1.5 md:gap-2">
                <UserIcon className="w-4 h-4 md:w-5 md:h-5 text-green-600" />
              ...
              <div className="flex items-start gap-2 md:gap-3">
                {profilePic ? (
                  <img 
                    src={profilePic} 
                    alt={farmerName}
                    className="w-10 h-10 md:w-12 md:h-12 rounded-full object-cover border-2 border-green-200 ring-1 ring-green-50 flex-shrink-0"
                  />
                ) : (
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-gradient-to-br from-green-400 to-emerald-500 flex items-center justify-center ring-1 ring-green-50 flex-shrink-0">
                    <UserIcon className="w-5 h-5 md:w-6 md:h-6 text-white" />
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                    <span className="font-bold text-xs md:text-sm text-slate-800 truncate">{farmerName}</span>
                    {isFarmerVerified ? (
                      <span className="px-1.5 py-0.5 bg-green-100 text-green-700 rounded-full text-[10px] md:text-xs font-bold flex items-center gap-0.5 flex-shrink-0">
                        <CheckBadgeIconSolid className="w-2.5 h-2.5 md:w-3 md:h-3" />
                        <span className="hidden sm:inline">Verified</span>
                  ...
                  {farmer?.rating && (
                    <div className="text-[10px] md:text-xs text-slate-600 mb-0.5">⭐ {farmer.rating.toFixed(1)} rating</div>
                  )}
                  {farmer?.phone && (
                    <div className="text-[10px] md:text-xs text-slate-600 truncate">📞 {farmer.phone}</div>
                  )}
                  <div className="flex items-center gap-1 mt-1 text-[10px] md:text-xs text-slate-600">
                    <MapPinIcon className="w-3 h-3 text-green-600 flex-shrink-0" />
          ...
          <div className="space-y-2">
            {showActions && item.status === 'available' && onBuyNow && (
              <button 
                ...
                className="w-full bg-gradient-to-r from-green-600 via-emerald-600 to-green-600 text-white py-2.5 md:py-3 rounded-lg md:rounded-xl font-bold text-sm md:text-base shadow-lg hover:shadow-xl hover:from-green-700 hover:via-emerald-700 hover:to-green-700 transition-all flex items-center justify-center gap-2 transform hover:scale-[1.02] active:scale-[0.98]"
              >
            ...
            <div className="mt-4 bg-white rounded-lg border-2 border-blue-200 shadow-lg overflow-hidden">
              ...
              {!matchId ? (
                <div className="p-4 text-center text-slate-500 text-sm">
                  ...
                  <p className="text-xs mt-1">Use "Buy Now" to create a match, then you can message here.</p>
                </div>
              ) : (
                <>
